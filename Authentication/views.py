...
class ResetPassView(UpdateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.save()
        return Response(
            {"messsage": ["Password changed successfully"]}, status=status.HTTP_200_OK
        )
...
class Sendotpphone(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        PhoneOTP.objects.update_or_create(contact=contact, defaults={"otp": otp})
        return Response(
...
class VerifyPhoneOTP(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.data["otp"] is None:
            return Response(
                {"success": "False", "message": "OTP is required to verify"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"success": "False", "error": "This OTP is not valid"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        otph = userotp.otp
        if otph != otp:
        ...
        if userotp.otp_created_at + timedelta(minutes=5) < timezone.now():
            return Response(
                {"success": "False", "error": "OTP expired, Resend OTP"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class UpdatecurrencyView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(currency) > 3:
            return Response(
                {"success": "False", "error": "Write a valid code"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class DeviceTokenView(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response(
                {"error": "FCM token is not provided."},
