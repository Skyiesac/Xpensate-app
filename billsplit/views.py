...
class AllGroupsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        groups = (
            Group.objects.filter(groupowner=request.user)
            .annotate(member_count=Count("groupmember"))
            .select_related("groupowner")
            .order_by("-created_at")
        )
        othergrps = (
            Group.objects.exclude(groupowner=request.user)
            .filter(groupmember__member=request.user)
            .select_related("groupowner")
            .annotate(member_count=Count("groupmember"))
            .order_by("-created_at")
        )
...
class AddGroupView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            Group.objects.filter(name=name, groupowner=request.user).exists()
            or Group.objects.filter(name=name, members=request.user).exists()
        ):
            return Response(
                {
                    "success": "False",
...
class AddMemberView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if member is None:
                return Response(
                    {"success": "False", "error": "User with email not found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            GroupMember.objects.create(group=group, member=member)
            return Response(
                {"success": "True", "message": "Member added to group successfully"},
                status=status.HTTP_201_CREATED,
            )
        else:
            return Response(
...
class RemovememberView(APIView):
    ...
    def delete(self, request, id, email, *args, **kwargs):
        ...
        if groupmember is None:
            return Response(
                {"success": "False", "error": "Group member not found"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        groupmember.delete()
        return Response(
            {"success": "True", "message": "Member removed from group successfully"},
...
class CreateBillView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not bill_serializer.is_valid():
            return Response(
                {"success": "False", "error": bill_serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class MarkAsPaidView(APIView):
    ...
    def post(self, request, id, *args, **kwargs):
        ...
        billparticipant = get_object_or_404(
            BillParticipant, bill=bill, participant=participant
        )
        ...
        email_for_paying(
            bill.billowner.email, email, billparticipant.amount, "Bill expenses"
        )
        return Response(
            {"success": "True", "message": "Participant marked as paid successfully"},
            status=status.HTTP_200_OK,
        )
...
class RecentsplitsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        user = request.user
        bills = (
            Bill.objects.filter(billparticipant__participant=user)
            .select_related("group")
            .prefetch_related("billparticipant_set__participant")
            .order_by("-billdate")
        )
...
class GroupDetailView(APIView):
    ...
    def get(self, request, id, *args, **kwargs):
        group = (
            ...
            .select_related("groupowner")
            .prefetch_related(
                "groupmember_set__member", "bill_set__billparticipant_set__participant"
            )
            .first()
        )
