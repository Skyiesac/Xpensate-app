...
class GroupSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        group = Group.objects.create(
            name=validated_data["name"], groupowner=request.user
        )
...
class BillSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for participant in participants_data:
            if participant["amount"] < 0:
                raise serializers.ValidationError(
                    "Participant's amount must not be negative"
                )
        ...
        bill = Bill.objects.create(
            billowner=billowner, billname=billname, **validated_data
        )
        bp = []
        for participant_data in participants_data:
            ...
            if not GroupMember.objects.filter(
                group=group, member=participant_user
            ).exists():
...
class BillgetSerializer(serializers.ModelSerializer):
    bill_participants = BillParticipantget(
        source="billparticipant_set", many=True, required=False
    )
